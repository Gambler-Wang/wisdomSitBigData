...
import {alarmListAPI,greenConstructionInfoAPI,greenConstructionItemInfo,specialDeviceListAPI,labelNameListAPI} from '@/config/api.js'
...
export const uploadFileRequest = (url, params) => {
  return axios({
    ...
    headers: {
      ...
      'smartdevice-auth': this.getCookie('smartdeviceToken'),
    }
  });
}
...
axios.interceptors.request.use(config => {
    if(!(config.url.indexOf(alarmListAPI) > -1 || 
      config.url.indexOf(greenConstructionInfoAPI) > -1 ||
      config.url.indexOf(greenConstructionItemInfo) > -1 ||   
      config.url.indexOf(specialDeviceListAPI) > -1 ||   
      config.url.indexOf(labelNameListAPI) > -1)     
    ){
      loaddingNum++
...
axios.interceptors.response.use(res=>{
  ...
  if(!(res.config.url.indexOf(alarmListAPI) > -1 || 
    res.config.url.indexOf(greenConstructionInfoAPI) > -1 ||
    res.config.url.indexOf(greenConstructionItemInfo) > -1 ||   
    res.config.url.indexOf(specialDeviceListAPI) > -1 ||   
    res.config.url.indexOf(labelNameListAPI) > -1)     
  ){
    loaddingNum--
    if (loaddingNum <= 0) {
